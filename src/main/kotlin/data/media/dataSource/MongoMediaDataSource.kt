...
class MongoMediaDataSource(
    ...
    override suspend fun getBookmarkedMediaListOfUser(email: String): List<Media>? {
        val bookmarkMediaList = userCollection.find(
            eq(User::email.name, email)
        ).firstOrNull()?.mediaList?.filter {
            it.isBookmarked
        }

        return bookmarkMediaList
    }

    override suspend fun upsertMediaToUser(email: String, media: Media): Boolean {
    ...
    override suspend fun deleteMediaFromUser(email: String, mediaId: Int): Boolean {
